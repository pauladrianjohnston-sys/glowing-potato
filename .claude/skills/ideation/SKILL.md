...
1. Read key existing artifacts to establish context:
...
11. **Trigger production** — When the Writer sends "All deliverables complete," spawn four production teammates (Image Agent, Presentation Agent, Web Page Agent, Archivist) and create production tasks with dependencies. See the **Production Phase** section below.
...
ideation-<slug>-<YYYYMMDD-HHMMSS>/
  ...
  images/                         # Infographic images (referenced by index.html)
...
findings. Use those findings to inform your editorial judgment. You may also
...
emphasize possibility. The Grounder would emphasize what connects to the brief. You have no
...
The vision document:
- Synthesizes all "interesting" ideas into a **unified product vision**, not
...
Image Agent, Presentation Agent, Web Page Agent, and Archivist all build their
artifacts from this document, not from individual briefs.
...
Write with clarity and neutrality. Do not favor the Free Thinker's enthusiasm
...
**Start by:** Reading the concept seed (the Arbiter will tell you where it is),
then initializing the ideation graph document from the template. Monitor for the
...
back with what matters, organized clearly.
...
Organize by relevance, not by source._
...
4. Produce the **vision document** at `{session-output}/session/VISION_<concept-slug>.md`
   ...
   entire session. It synthesizes the briefs into a unified vision and becomes
...
teammates who transform the Writer's output into distributable artifacts.

### One Content, Three Formats

The production phase produces three presentation artifacts from the vision
...
A fourth artifact — the **Session Capsule PDF** — is different in kind. It is
...
1. Creates the production output subdirectories (if not already created in
   Step 2):
   ```bash
   mkdir -p {session-output}/images {session-output}/build
   ```
...
3. Creates four production tasks with dependencies:
   ...
   ```
4. The Image Agent and Presentation Agent work **in parallel** (tasks A and B
   are unblocked)
...
2. **Craft an image generation prompt** — describe the concept as a visual
   infographic. Include:
   - The central idea and its key components
   - Relationships and connections between elements
   - Visual metaphors that capture the essence of the idea
   - Style direction: clean, informative infographic — not decorative art
3. **Navigate to ChatGPT in Chrome** — use `mcp__claude-in-chrome__navigate`
   to open ChatGPT, then use the form input and interaction tools to submit
   the image generation prompt
4. **Wait for and download the generated image** — use the browser tools to
   locate the generated image and download it
5. **Save to `{session-output}/images/`** — use a descriptive filename, e.g.,
   `INFOGRAPHIC_<idea-slug>.png`

When all images are complete, send a `message` to the team lead confirming:
**"All infographic images complete"** and list the files produced.
...
serves as the primary distribution artifact for this ideation session.** You
use the `frontend-design` skill approach — creative, high-quality web design
that avoids generic AI aesthetics.

**IMPORTANT: You are blocked until the Image Agent and Presentation Agent
...
2. Wait for your task to become unblocked (Image and Presentation agents must
   complete first)
...
- **Card or section layout for each idea/move** — one visual section per idea,
  ...
  - The infographic image (referenced via relative path
    `images/INFOGRAPHIC_<slug>.png`)
  - Key framings and design decisions
...
- **Visual polish** — good typography, readable layout, cohesive color scheme,
  responsive design. This is the artifact people will actually look at — it
...
document to plan your layout. But don't write the HTML until the images and
presentation are done so you know exactly what files to reference.
...
**Your job is to produce two PDF artifacts that package the session's output
...
3. For the Results PDF: injects print-friendly `@page` CSS into the
   distribution page HTML, converts image paths to base64 data URIs (so the
   PDF is self-contained), and renders to PDF via `weasyprint`
4. For the Capsule PDF: generates styled HTML from all session artifacts with
   inline CSS and base64-embedded images, then renders to PDF
...
someone" artifact — email it, attach it, archive it. Same content, same
design, PDF format.
...
2. Convert all relative image paths (`images/*.png`) and CSS `url()`
   references to base64 data URIs so the PDF is fully self-contained
...
4. **Neutralize scroll-reveal animations.** The Web Page Agent's distribution
   ...
   - Set `opacity: 1 !important` on any class that initializes at `opacity: 0`
...
review. The "everything in one place" artifact.
...
|---------|----------|-------------|
| **Cover** | Session topic (large), primary recommendation, thesis line, date, lead infographic thumbnail | Vision document header |
...
| **Layer 3: Exploration** | Each brief in full, infographic images, research findings (if Explorer was active) | `session/briefs/*.md`, `images/*.png`, `session/research/*.md` |
| **Layer 4: Origins** | Original user request, all captured source materials at full fidelity, images at readable size with text descriptions | `session/sources/*` |
...
- **"The capsule is the frame. The content is the art."** — the design is
  ...
  identity.
- **Temperature-neutral** — the capsule doesn't editorialize. It presents what
...
Not every session produces all possible artifacts. The build script should:
...
1. Send `shutdown_request` via `SendMessage` to **all active teammates** — this
   ...
   (if spawned), and the production teammates (Image Agent, Presentation Agent,
